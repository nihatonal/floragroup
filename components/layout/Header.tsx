  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? 'bg-flora-ivory/90 text-flora-forest shadow-soft backdrop-blur-2xl'
          : 'bg-flora-ivory/70 backdrop-blur-[20px]supports-[backdrop-filter]:bg-flora-ivory/60 text-flora-forest shadow-soft backdrop-blur-3xl'
      )}
    >
      <Container
        className={cn(
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            ...
            <motion.div
              className="fixed bottom-0 right-0 top-0 z-[60] w-full max-w-[430px] overflow-hidden bg-flora-ivory text-flora-forest shadow-premium lg:hidden"
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
