...
export default function WhyChooseUs() {
  ...
  return (
    <section className="relative overflow-hidden bg-flora-forest py-16 text-flora-ivory md:py-24">
            ...
            {items.map((item, index) => {
              ...
              return (
                <StaggerItem key={item.key}>
                  <article
                    className={`group relative overflow-hidden border border-flora-ivory/8 bg-flora-night/18 p-7 transition duration-500 hover:border-flora-champagne/35 hover:bg-flora-night/28 ${
                      index === 1 ? "lg:mt-12" : ""
                    } ${index === 2 ? "lg:-mt-6" : ""}`}
                  >
                    ...
                    <h3 className="mt-7 font-display text-2xl leading-none tracking-[-0.04em]">
                      {t(`items.${item.key}.title`)}
                    </h3>

                    <p className="mt-4 text-sm leading-6 text-flora-ivory/58">
